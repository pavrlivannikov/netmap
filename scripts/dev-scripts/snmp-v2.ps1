...
# System info
$sn = sget "1.3.6.1.2.1.1.5.0"
$sd = sget "1.3.6.1.2.1.1.1.0"
$sl = sget "1.3.6.1.2.1.1.6.0"
Write-Host "Name:     $sn" -ForegroundColor Green
Write-Host "Descr:    $sd" -ForegroundColor Green
Write-Host "Location: $sl" -ForegroundColor Green
Write-Host ""
...
for ($i = 1; $i -le 28; $i++) {
    ...
    $o = sget "1.3.6.1.2.1.2.2.1.8.$i"
    if ($n) {
        $st = "DOWN"; if ($o -eq 1) { $st = "UP" }
        Write-Host "  $i [$st] $n"
    }
}
